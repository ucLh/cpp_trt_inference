...
TRTCNNInferencer::TRTCNNInferencer()
{
    ...
    _network_definition = samplesCommon::InferObject(_builder->createNetworkV2(explicitBatch));

    _onnx_parser = samplesCommon::InferObject(nvonnxparser::createParser(*_network_definition, gLogger));
...
void TRTCNNInferencer::configureGPUMemory()
{
    ...
    if (err == cudaSuccess) {
        ...
        if (bytes_fraction > free_bytes) {
            std::cerr << "Warning, desired GPU memoryy fraction is lower than free memory available. Will be set free fraction" << std::endl;
...
bool TRTCNNInferencer::loadFromONNX(const std::string &filename)
{
    ...
    bool success = _onnx_parser->parseFromFile(filename.c_str(), static_cast<int>(gLogger.getReportableSeverity()));
    //, *_network_definition, DataType::kFLOAT);
    if (!success) {
        _last_error = "Can't parse model file " + std::string(_onnx_parser->getError(0)->desc());
        return false;
    }

    auto prof = _builder->createOptimizationProfile();

    prof->setDimensions(getInputNodeName().c_str(), OptProfileSelector::kMIN, nvinfer1::Dims4{(int)_batch_size,
                                                                                              getInputHeight(),
                                                                                              getInputWidth(), 3});
    prof->setDimensions(getInputNodeName().c_str(), OptProfileSelector::kMAX, nvinfer1::Dims4{(int)_batch_size,
                                                                                              getInputHeight(),
                                                                                              getInputWidth(), 3});
    prof->setDimensions(getInputNodeName().c_str(), OptProfileSelector::kOPT, nvinfer1::Dims4{(int)_batch_size,
                                                                                              getInputHeight(),
                                                                                              getInputWidth(), 3});

    _builder_config->addOptimizationProfile(prof);

    _cuda_engine = samplesCommon::InferObject(_builder->buildEngineWithConfig(*_network_definition, *_builder_config));
    ...
    _context->setBindingDimensions(0, nvinfer1::Dims4{(int)_batch_size, getInputHeight(), getInputWidth(), 3});
...
bool TRTCNNInferencer::loadFromCudaEngine(const string &filename)
{
    ...
    if (filename.rfind(".ce") == std::string::npos && filename.rfind(".bin") == std::string::npos) {
    ...
    _cuda_engine = samplesCommon::InferObject(_runtime->deserializeCudaEngine(buffer.data(), buffer.size()));
    ...
    // Create RAII buffer manager object
    _buffers = std::shared_ptr<samplesCommon::BufferManager>(new samplesCommon::BufferManager(_cuda_engine, _batch_size));
...
bool TRTCNNInferencer::loadFromUff(const string &filename)
{
    ...
    _uff_parser->registerInput(_input_node_name.c_str(), _input_shape, nvuffparser::UffInputOrder::kNHWC);
    ...
    bool success = _uff_parser->parse(filename.c_str(), *_network_definition, DataType::kFLOAT);
    if (!success) {
...
std::string TRTCNNInferencer::inference(const std::vector<cv::Mat> &imgs)
{
    ...
    bool success = processInput(*_buffers, imgs, {0.0, 0.0, 0.0}, _norm_type, _bgr2rgb);
    ...
    bool status = _context->execute(_batch_size, _buffers->getDeviceBindings().data());
...
int TRTCNNInferencer::getInputHeight() const
{
    return _input_shape.d[1];
}
...
int TRTCNNInferencer::getInputWidth() const
{
    return _input_shape.d[2];
}
...
int TRTCNNInferencer::getInputDepth() const
{
    return _input_shape.d[0];
}
...
std::string TRTCNNInferencer::getLastError() const
{
    return _last_error;
}
                                    ...
                                    NormalizeType normalize, bool rgb)
{
    ...
    float* hostDataBuffer = static_cast<float*>(buffers.getHostBuffer(_input_node_name));
    ...
    for (int i = 0; i < _batch_size; ++i) {
        ...
        cv::resize(imgs[i], input_img, cv::Size(inputW, inputH), 0, 0, cv::INTER_LINEAR);

        // positions - height - width
        input_img.forEach<cv::Vec3b>([&](cv::Vec3b& pixel, const int position[]) -> void {
            ...
            for (short c = 0; c < inputC; ++c) {
                ...
                if (normalize == NormalizeType::DETECTION) {
                    ...
                    // WARN: IDK why, this shouldnt happen, but work only with this preprocessing
                    val = float(val);
                    val /= 255.0;
                    //val -= 0.5;
                    //val *= 2.0;
                ...
                int pos = 0;
                if (rgb) {
                    pos = i * volImg + (2 - c) * volChl + position[0] * inputH + position[1];
                }
                else {
...
bool TRTCNNInferencer::getBGR2RGBConvertionEnabled() const
{
    return _bgr2rgb;
}
