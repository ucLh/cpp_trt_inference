...
#ifdef TRT_DEBUG
std::vector<cv::Mat> TRTDetectionInferencer::getFramesWithBoundingBoxes(float tresh)
{
    if (tresh == 0.0f) {
        tresh = _thresh;
    }
...
bool TRTDetectionInferencer::processOutput(const samplesCommon::BufferManager &buffers)
{
    float* hostDataBuffer = static_cast<float*>(buffers.getHostBuffer(_output_node_names[0]));
    // NOTE: buffers.size give bytes, not lenght, be carefull
    const size_t size = (buffers.size(_output_node_names[0]) / sizeof(float)) / _batch_size;
    ...
    for (size_t example_num = 0; example_num < _batch_size; ++example_num) {
        ...
        for (size_t index = 0; index < number_of_detections; ++index) {
            const size_t prefix = (example_num * number_of_detections) + index * _layout_size;

//            std::cout << hostDataBuffer[prefix +0] << " " << hostDataBuffer[prefix + 1] << " " << hostDataBuffer[prefix +2] << " "
//                                                      << hostDataBuffer[prefix +3] << " "<< hostDataBuffer[prefix +4] << " "
//                                                      << hostDataBuffer[prefix +5] << " " << hostDataBuffer[prefix +6] << std::endl;
            ...
            _boxes[example_num][index] = cv::Rect2f(xmin, ymin, xmax - xmin, ymax - ymin);
...
float TRTDetectionInferencer::getThresh() const
{
    return _thresh;
}

void TRTDetectionInferencer::setThresh(float thresh)
{
    _thresh = thresh;
}
