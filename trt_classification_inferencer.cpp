...
string TRTClassificationInferencer::inference(const std::vector<cv::Mat> &imgs)
{
...
bool TRTClassificationInferencer::processOutput(const samplesCommon::BufferManager &buffers)
{
    ...
    const size_t num_of_classes = (buffers.size(_output_node_names[0]) / sizeof(float)) / _batch_size;
...
float TRTClassificationInferencer::getThresh() const
{
    return _thresh;
}

void TRTClassificationInferencer::setThresh(float thresh)
{
    _thresh = thresh;
}
